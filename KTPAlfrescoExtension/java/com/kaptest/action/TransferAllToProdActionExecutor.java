...
public class TransferAllToProdActionExecutor extends ActionExecuterAbstractBase
{   
	...
	protected void executeImpl(Action action, NodeRef actionedUponNodeRef){
		System.out.println("Ready for Transfer Service to Prod");		
        ...
        if(nodesInTree!=null){
			...
			try{
				System.out.println("Start All Transfer to PROD Repository");
				TransferEndEvent transferEvent = transferService.transfer("FSTR_PROD", transferDef);				
				if(transferEvent.getMessage().equalsIgnoreCase("success")){
					System.out.println("Start Transfer to FSTR_PROD is Successful");	
					for(NodeRef nodeRef : nodesInTree){
						...
						if(nodeService.hasAspect(nodeRef, KaptestModel.ASPECT_IS_READY_TO_PUBLISH)){
					       	nodeService.removeAspect(nodeRef, KaptestModel.ASPECT_IS_READY_TO_PUBLISH);
					    }
						if(!nodeService.hasAspect(nodeRef, KaptestModel.ASPECT_IS_PUBLISHED)){
							...
							propMap.put(KaptestModel.PROPERTY_KTP_ID, "INITIALOAD");
        	...
        	System.out.println("No Node To Transfer to PROD");
        }
